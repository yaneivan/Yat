...
## 🐛 БАГИ (исправлять СРОЧНО)

### Кнопка "Назад" пропадает при переключении изображений в cropper

**Статус:** ❌ Не исправлено
**Приоритет:** 🔴 КРИТИЧНО
**Время на фикс:** 30 минут
**Риск:** Плохой UX — пользователь не может вернуться в проект

**Проблема:**
В `templates/cropper.html` кнопка "📋 Проект" находится на строке 103:
```html
<button class="btn" onclick="window.location.href='/project/{{ project }}'" title="Назад к проекту">📋 Проект</button>
```

При переключении между изображениями (next/prev) кнопка пропадает.

**Решение:**
- Проверить JS код переключения изображений
- Убедиться что toolbar не перерисовывается полностью
- Или явно обновлять кнопку после переключения

**Файлы для изменения:**
- `templates/cropper.html`
- `static/js/cropper.js` (если есть)
