...
### Переименование статуса "Размечено"

**Описание:**
Текущие статусы:
- Загружено
- Кадрировано
- **Размечено** ← переименовать
- Текст добавлен
- Требует ревью
- Проверено

**Новое название:** "Добавлены полигоны" (более понятно)

**Что изменить:**
- `database/enums.py` — значение в `ImageStatus.SEGMENT`
- `static/js/status_widget.js` — отображение в UI
- `templates/*.html` — если есть тексты
- Перевод в `status_widget.py` (если есть)

**Файлы для изменения:**
- `database/enums.py`
- `static/js/status_widget.js`
- `services/image_service.py` (логика статусов)

**Ветка:** `feature/rename-segment-status`
...
**Описание:**
Сейчас при переходе на несуществующий URL (например `/foobar` или `/editor?image=nonexistent`) Flask показывает стандартную белую страницу "404 Not Found".

**Что сделать:**
- Добавить обработчик `@app.errorhandler(404)` в `app.py`
- Создать `templates/404.html` с красивым дизайном
- Добавить кнопку "На главную" и, возможно, навигацию

**Файлы для изменения:**
- `app.py` (добавить errorhandler)
...
| 4 | Автосохранение | 🟠 | 1 ч | ❌ |
| 5 | Индикатор загрузки | 🟠 | 2 ч | ❌ |
| 6 | Хардкод путей | 🟢 | 30 мин | ❌ |
| 7 | Тесты AI | 🟢 | 4 ч | ❌ |
| 8 | Magic numbers | 🟢 | 30 мин | ❌ |
| 9 | Форматирование текста | 🟠 | 2 ч | ❌ |
| 10 | Копирование изображения | 🟢 | 30 мин | ❌ |
| 11 | Унификация фоновых задач | 🟠 | 3 ч | ❌ |
| 12 | Ленивая инициализация TROCR | 🟡 | 1 ч | ❌ |
| 13 | Исправление математики координат | 🔴 | 4 ч | ❌ |
| 14 | Страница 404 | 🟢 | 30 мин | ❌ |

**Общее время на критические исправления:** ~6 часов
**Общее время на все исправления:** ~19.5 часов
