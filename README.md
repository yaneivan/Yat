...
https://github.com/user-attachments/assets/b4c7713f-214b-47ac-a743-437c7b94b363
...
2. **Выберите конфигурацию PyTorch под вашу систему** (отредактируйте `pyproject.toml`):
   
   ```toml
   # ВАРИАНТ 1: CPU (без видеокарты NVIDIA)
   "torch>=2.5.0",
   "torchvision>=0.20.0",
   
   # ВАРИАНТ 2: CUDA 12.1 (драйверы 470.x - 535.x)
   # Для: P102, GTX 10xx, RTX 20xx, RTX 30xx (старые драйверы)
   "torch==2.5.1+cu121",
   "torchvision==0.20.1+cu121",
   
   # ВАРИАНТ 3: CUDA 12.4 (драйверы 545.x+)
   # Для: GTX 16xx, RTX 30xx, RTX 40xx (новые драйверы)
   "torch==2.6.0+cu124",
   "torchvision==0.21.0+cu124",
   ```
   
   > 💡 **Проверить версию драйвера:** `nvidia-smi` (смотрите столбец "Driver Version")
   
   > ⚠️ **Важно:** Раскомментируйте только ОДИН вариант, остальные закомментируйте
...
4. Инициализируйте базу данных (если не создана автоматически):
   ```bash
