...
1. Создайте папку /data в корне проекта. 
    Подпапки создадутся автоматически при запуске проекта. 
...
- При переещении по стрелочкам в редакторе обрезки - пропадает кнопка выхода в проект обратно. только при входе с проекта кнопка возвращаения видна. Проблема только в редакторе обрезки. При переходе из других редакторов - кнопка возврата в проект видна. Также можно добавить слеш для отделения домика и проект от навигации по редакторам.
- Кнопка распознать все полигоны не работает.  
- Возможно проблемы при разработке в докере. 